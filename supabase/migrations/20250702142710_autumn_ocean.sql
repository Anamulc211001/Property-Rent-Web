/*
  ...
  1. New Tables
    - `users` - User profiles with additional information beyond auth.users
    - Ensure all other tables reference this users table properly
  ...
  3. Relationships
    - Fix all foreign key relationships to reference public.users
...
-- Create listings table with proper foreign keys
...
DECLARE
  ...
  sample_area_id uuid;
BEGIN
  -- Only add sample data if no listings exist
  IF NOT EXISTS (SELECT 1 FROM listings LIMIT 1) THEN
    -- Get a sample area
    SELECT id INTO sample_area_id FROM areas WHERE name = 'খুলশী' LIMIT 1;
    
    IF sample_area_id IS NOT NULL THEN
      -- Create a sample listing (this will only work if there are authenticated users)
      -- This is just for demonstration - real listings will be created by users
      INSERT INTO listings (
        owner_id, title, description, rent, advance, category, rooms, bathrooms,
        furnishing, facilities, area_id, address, size, available_from
      ) VALUES (
        '00000000-0000-0000-0000-000000000000', -- Placeholder - will be replaced by real user IDs
        'খুলশীতে সুন্দর ২ রুমের ফ্ল্যাট',
        'খুলশী এলাকায় অবস্থিত একটি সুন্দর ২ রুমের ফ্ল্যাট। সব ধরনের সুবিধা সহ।',
        15000, 30000, 'flat', 2, 1, 'semi_furnished',
        '["বিদ্যুৎ", "পানি", "গ্যাস", "ইন্টারনেট"]'::jsonb,
        sample_area_id, 'খুলশী, চট্টগ্রাম', 800, CURRENT_DATE
